...
## שני מצבי עבודה
פאנל הניהול (`admin.html`) עובד בשני מצבים — אוטומטית:

| | **מצב מקומי** (ברירת מחדל, ללא הקמה) | **מצב Supabase** (אחרי ההקמה למטה) |
|---|---|---|
| כניסה | סיסמה שנקבעת בכניסה הראשונה (בדפדפן) | התחברות מאובטחת אמיתית |
| שמירה | בדפדפן הנוכחי (localStorage) | בענן — מתעדכן לכל הגולשים מיד |
| תמונות | מוקטנות ונשמרות מקומית | מועלות לאחסון בענן |
| מתאים ל | התנסות / עריכה מהירה | אתר חי בפרודקשן |

**להתחלה מיידית (מצב מקומי):** הריצו `node serve.js` ופתחו `http://localhost:4173/admin.html` — הזינו סיסמה (פעם ראשונה קובעת אותה) ותתחילו לנהל. השינויים נשמרים ומופיעים מיד באתר באותו דפדפן. כפתור **„ייצוא קטלוג”** מוריד את הנתונים לקובץ.

**למצב המאובטח (Supabase)** — השלימו את ההקמה הבאה; ברגע שתזינו מפתחות, הפאנל והאתר עוברים אוטומטית לענן.
